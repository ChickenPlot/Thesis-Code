...
gwas_areb3 <- read_csv('spain_relict/areb3_summary_stats.csv', 
                       #header = T, 
                       skip = 8)


gwas_eel <- read_csv('spain_relict/eel_summary_stats.csv',
                     #header = T,
                     skip = 8)


gwas_fd <- read_csv('spain_relict/fd_summary_stats.csv', 
                    #header = T,
                    skip = 8)


gwas_fdp <- read_csv('spain_relict/fdp_summary_stats.csv', 
                     #header = T, 
                     skip = 8)
```
...
length(which(gwas_areb3$`Benjamini-Hochberg-P-Value` <= 0.2)) # 0 datapoints
#length(which(gwas_areb3$`Benjamini-Hochberg-P-Value` > 0.2))
...
ggplot(data = world) +
    geom_sf() + 
    coord_sf(xlim = long_lim, ylim = lat_lim, expand = T)+
  geom_point(data = minor_hap_eel, aes(x = longitude ,y =latitude), color = '#3ac9ba', size = 4, alpha = 0.9) + theme_minimal()
```
